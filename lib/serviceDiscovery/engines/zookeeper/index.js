...
/**
 * Retrieve the announce path
 */
ZooKeeperService.prototype.getAnnouncePath = function () {
	if (this.port === null) {
		throw new Error('Cannot retrieve the announce path before announcing');
	}

	return [this.baseAnnouncePath, os.hostname() + ':' + this.port].join('/');
};
...
ZooKeeperService.prototype.announce = function (port, metadata, cb) {
	...
	var announcePath = this.getAnnouncePath();
...
ZooKeeperService.prototype.close = function (callback) {
	...
	if (this.port && state === zooKeeper.State.SYNC_CONNECTED) {
		// Because MAGE does not gracefully quit (that is, quit once all refs are cleared),
		// we end up in a situation where `close` might not execute; in such cases, we end up
		// seeing zookeeper nodes linger for the duration of the session (default: 30 seconds)
		//
		// To avoid this issue, we force-delete the announce path on close
		this.client.remove(this.getAnnouncePath(), (error) => {
			this.client.close();
			callback(error);
		});
	} else if (callback) {
